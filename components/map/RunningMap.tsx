    ...
    <div className="relative h-[46vh] min-h-[320px] w-full overflow-hidden rounded-2xl border border-slate-200">
      ...
      {onResetCourse ? (
        <button
          ...
          className="absolute left-3 top-3 z-20 inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50"
          aria-label="코스 리셋"
          title="코스 리셋"
        >
      ...
      {mapError ? (
        <div className="absolute inset-x-3 top-14 z-10 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
          {mapError}
        </div>
      ) : null}
    </div>
